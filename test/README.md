...
| POST | `/tokens-sse` | Anthropic-shaped `text/event-stream`: 120 ms before the first event, then `message_start` (`input_tokens`), 3 deltas 60 ms apart, `message_delta` (`output_tokens`) | TS1/TS4 + UI4-UI7: measurement readout, modal layout, beautified stream body |
